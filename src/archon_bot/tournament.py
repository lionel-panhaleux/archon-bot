...
class Score:
    ...
    def __add__(self, rhs):
        return self.__class__(gw=self.gw + rhs.gw, vp=self.vp + rhs.vp, tp=self.tp + rhs.tp)
...
class Tournament:
    ...
    async def add_player(
        ...
        if not vekn:
            if self.flags & TournamentFlag.VEKN_REQUIRED and not judge:
                raise CommandFailed("Only a judge can register a player without VEKN ID#")
            else:
        ...
        if vekn in self.players:
            if vekn in self.dropped:
                if not judge and self.dropped[vekn] == DropReason.DISQUALIFIED:
                    raise CommandFailed("Player was disqualified: only a judge can reinstate them")
            ...
            if self.flags & TournamentFlag.STAGGERED:
                raise CommandFailed("Tournament is staggered, no more registration allowed.")
        ...
        if prev_vekn and prev_vekn in self.dropped:
            if not judge and self.dropped[prev_vekn] == DropReason.DISQUALIFIED:
                raise CommandFailed("Player was disqualified: only a judge can change their VEKN#")
    ...
    def add_player_deck(
        ...
        if not judge and self.current_round and not self.flags & TournamentFlag.MULTIDECK:
        ...
        if not judge and self.state == TournamentState.PLAYING and player.playing:
            raise CommandFailed("Round in progress: wait for it to end to change your deck")
    ...
    def check_deck(
        ...
        if self.include:
            excluded = [c.name for c, _ in deck.cards(lambda c: c.id not in self.include)]
            if any(excluded):
                issues.append(DeckIssue.ExcludedCards(excluded))
        if self.flags & TournamentFlag.SINGLE_CLAN:
            clans = collections.Counter([c.clan for c, _ in deck.cards(lambda c: c.crypt)])
    ...
    async def start_round(self, progression_callback: Callable | None = None) -> Round:
        ...
        if self.current_round > 1:
            optimised_rounds, score = await asgiref.sync.sync_to_async(krcg.seating.optimise)(
    ...
    async def make_staggered(self, rounds_count: int, progression_callback: Callable) -> None:
        ...
        if len(players) not in [6, 7, 11]:
            raise CommandFailed("A staggered tournament requires exactly 6, 7 or 11 players")
    ...
    def unmake_staggered(self) -> None:
        ...
        if self.current_round:
            raise CommandFailed("The tournament has started: too late to modify its structure")
        self.rounds = []
        self.flags ^= TournamentFlag.STAGGERED
    ...
    def reset_round(self) -> Round:
        ...
        if self.rounds[-1].results:
            raise CommandFailed("Some rounds results have been entered, the round cannot be reset.")
    ...
    def standings(self, toss=False) -> Tuple[Optional[str], list[Rank]]:
        ...
        for i, round in enumerate(
            ...
            if incorrect:
                if len(incorrect) > 1:
                    raise CommandFailed(f"Incorrect score for tables {incorrect} in round {i}")
                if len(incorrect) > 0:
                    raise CommandFailed(f"Incorrect score for table {incorrect[0]} in round {i}")
    ...
    def start_finals(self) -> Round:
        ...
        self.current_round += 1
        self.rounds.append(Round(finals=True, seating=krcg.seating.Round.from_players(top_5)))
        self.state = TournamentState.PLAYING
        return self.rounds[-1]
    ...
    def validate_score(
        ...
        if table_number < 1 or table_number > len(round.seating):
            raise CommandFailed("Invalid table number")
        round.overrides[table_number] = Note(level=NoteLevel.OVERRIDE, judge=judge, text=comment)
    ...
    def player_status(self, vekn: str):
        ...
        if self.state == TournamentState.CHECKIN:
            if player.playing:
                return PlayerStatus.CHECKED_IN
            elif self.max_rounds and self.player_rounds_played(player) >= self.max_rounds:
                return PlayerStatus.MAX_ROUNDS_PLAYED
            else:
